 ...
 * 3. Lambda verifies token and checks student/instructor/admin group membership
...
async function initializeConnection() {
  try {
    ...
    // Create JWT verifier configured to validate student, instructor, or admin group membership
    jwtVerifier = CognitoJwtVerifier.create({
      userPoolId: credentials.VITE_COGNITO_USER_POOL_ID,
      tokenUse: "id", // Validate ID tokens (not access tokens)
      groups: ["student", "instructor", "admin"], // Allow all authenticated user groups
